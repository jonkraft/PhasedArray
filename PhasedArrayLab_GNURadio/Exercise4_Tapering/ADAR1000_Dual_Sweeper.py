...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def work(self, input_items, output_items):
        ...
        if self.Taper==0:
            ...
            self.spi.xfer2([ADDR1, 0x10, int(128+self.RxGain1)])  # Sets Rx1 VGA gain
            self.spi.xfer2([ADDR1, 0x11, int(128+self.RxGain2)])  # Sets Rx2 VGA gain
            self.spi.xfer2([ADDR1, 0x12, int(128+self.RxGain3)])  # Sets Rx3 VGA gain
            if self.SymTaper==0:
                self.spi.xfer2([ADDR1, 0x13, int(128+self.RxGain4)])  # Sets Rx4 VGA gain
            else:
                self.spi.xfer2([ADDR1, 0x13, int(128+self.RxGain1)])  # If Symmetric Taper is set high, then make Rx4 gain equal to Rx1 gain
        else:
